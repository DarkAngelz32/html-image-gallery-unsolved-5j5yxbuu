...
const testDetailPageStructure = (
  ...
  const testMainImage = () => {
    ...
    expect(checkFileExists(imageSrc)).toBe(true);
  };
...
describe('Modern Design Page Tests', () => {
  ...
  describe('Modern Design Page: Basic structure and connections', () => {
    ...
    describe('Test CSS connections', () => {
      ...
      test('image-details.css should be the second css file connected', () => {
        const cssLinks = page.locator('link[rel="stylesheet"]');
        const detailCssPath = cssLinks[1]?.getAttribute('href') || '';

        expect(detailCssPath.includes('/secondary/image-details.css')).toBe(
          true
        );
        expect(checkFileExists(detailCssPath)).toBe(true);
      });
    ...
    describe('Test main Image', () => {
      test('Main image should exist', () => {
        const images = page.locator('img');
        expect(images.length).toBeGreaterThan(0);
      });

      test('image src should be the expected file', () => {
        const images = page.locator('img');
        const imageSrc = images[0]?.getAttribute('src') || '';
    ...
    describe('Test Home link button', () => {
      test('Should find a link that has the text "Back to Home Page"', () => {
        const allLinks = page.locator('a');
        const backHomeLink = allLinks.find((link) =>
          link.text?.trim()?.includes('Back to Home Page')
        );
      ...
      test('the link should have a relative path back to index.html', () => {
        const allLinks = page.locator('a');
        const backHomeLink = allLinks.find((link) =>
          link.text?.trim()?.includes('Back to Home Page')
        );
    ...
    describe('Main Navigation links work correctly', () => {
      test('Should be a link to nature page', () => {
        const navLinks = page.locator('nav a');
        const natureLink = navLinks.find(
          (link) => link.text?.trim() === 'Nature'
        );
        const natureHref = preparePath(natureLink?.getAttribute('href'));
        expect([urls.naturePageUrl, urls.naturePageUrlPagesRel]).toContain(
          natureHref
      ...
      test('Should be a link to space page', () => {
        const navLinks = page.locator('nav a');
        const spaceLink = navLinks.find(
          (link) => link.text?.trim() === 'Space'
        );
        const spaceHref = preparePath(spaceLink?.getAttribute('href'));
      ...
      test('Should be a link to plants page', () => {
        const navLinks = page.locator('nav a');
        const plantsLink = navLinks.find(
          (link) => link.text?.trim() === 'Plants'
        );
        const plantsHref = preparePath(plantsLink?.getAttribute('href'));
        expect([urls.plantsPageUrl, urls.plantsPageUrlPagesRel]).toContain(
          plantsHref
      ...
      test('Should be a link to Modern Design Page page', () => {
        const navLinks = page.locator('nav a');
        const modernDesignLink = navLinks.find(
          (link) => link.text?.trim() === 'Modern Design'
        );
